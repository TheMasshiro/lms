...
const enrollment = () => {
  const { enrolledCourses, navigate, calculateCourseDuration, calculateNoOfLectures, backendUrl, userData, fetchUserEnrolledCourses, getToken } = useContext(AppContext)
  ...
  {enrolledCourses.map((course, index) => (
          <tr key={index} className="border-b border-gray-500/20">
              ...
              <td className="px-4 py-3 max-sm:text-right">
                  <button onClick={() => navigate('/vPlayer/' + course._id)} className='px-3 sm:px-5 py-1.5 sm:py-2 bg-blue-600 max-sm:text-xs text-white'>
